  ...
  module Repo
    class Occurrence
      ...
      def all_for_section(counting, section)
        Entity.where(counting_id: counting.id, sample_id: section.samples.map(&:id)).association_join(:species).order { rank.desc }.to_a
      end
      ...
      class Entity < Sequel::Model(Config.db[:occurrences])
        ...
        many_to_many :sections, class: 'Paleolog::Repo::Section::Entity', left_key: :sample_id, right_key: :section_id, join_table: :samples
      end
    end
  end
end
