...
describe 'Sections' do
  ...
  let(:project) do
    project, errors = Paleolog::Operation::Project.create(name: 'some project', user_id: user.id)
  ...
  describe 'POST /api/:project_id/sections' do
    ...
    describe 'with user' do
      ...
      it 'creates section and returns its attributes' do
        ...
        post "/api/#{project.id}/sections", params
  ...
  describe 'PATCH /api/:project_id/sections/:id' do
    it 'rejects guest access' do
      assert_unauthorized(-> { patch "/api/#{project.id}/sections/1", {} })
    end

    it 'rejects user observing the project' do
      Paleolog::Repo::ResearchParticipation.update(researcher.id, manager: false)
      login(user)
      assert_forbidden(-> { patch "/api/#{project.id}/sections/1", {} })
    end
    ...
    describe 'with user' do
      ...
      it 'updates section name and returns its attributes' do
        params = { 'id' => '1', 'name' => 'some other name' }
        patch "/api/#{project.id}/sections/#{section.id}", params
        assert_equal 200, last_response.status
      ...
      it 'returns errors in case of failure' do
        patch "/api/#{project.id}/sections/#{section.id}", {}
