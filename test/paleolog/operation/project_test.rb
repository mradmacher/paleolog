...
describe Paleolog::Operation::Project do
  ...
  describe '#find_all_for_user' do
    it 'returns unauthenticated error when not authenticated' do
      ...
      result = operation.find_all_for_user(user.id, authorizer: authorizer)
  ...
  describe '#create' do
    it 'returns unauthenticated error when not authenticated' do
      ...
      result = operation.create({ name: 'Just a Name', user_id: user.id }, authorizer: authorizer)
      assert_predicate result, :failure?
      assert_equal Paleolog::Operation::UNAUTHENTICATED, result.error[:general]
    ...
    describe 'for authorized user' do
      ...
      it 'adds new project' do
        result = operation.create({ name: 'Some Name', user_id: user.id }, authorizer: authorizer)
        assert_predicate result, :success?
      ...
      it 'complains when user missing' do
        result = operation.create({ name: 'Some Name', user_id: nil }, authorizer: authorizer)
        assert_predicate result, :failure?
        assert_equal ParamParam::NON_INTEGER, result.error[:user_id]
      end

      it 'does not complain when name not taken yet' do
        result = operation.create({ name: 'Some Name', user_id: user.id }, authorizer: HappyAuthorizer.new)
        assert_predicate result, :success?
        assert_equal 'Some Name', result.value.name

        result = operation.create({ name: 'Other Name', user_id: user.id }, authorizer: authorizer)
        assert_predicate result, :success?
        assert_equal 'Other Name', result.value.name
      end

      it 'complains when name is nil' do
        result = operation.create({ name: nil, user_id: user.id }, authorizer: authorizer)
        assert_predicate result, :failure?
        assert_equal :blank, result.error[:name]
      end

      it 'complains when name is blank' do
        result = operation.create({ name: '  ', user_id: user.id }, authorizer: authorizer)
        assert_predicate result, :failure?
        assert_equal :blank, result.error[:name]
      end
      ...
      it 'complains when name is too long' do
        max = 255
        result = operation.create({ name: 'a' * (max + 1), user_id: user.id }, authorizer: authorizer)
        assert_predicate result, :failure?
      ...
      it 'accepts name of max length' do
        name = 'a' * 255
        result = operation.create({ name: name, user_id: user.id }, authorizer: authorizer)
        assert_predicate result, :success?
  ...
  describe '#rename' do
    ...
    it 'returns unauthenticated error when not authenticated' do
      ...
      result = operation.rename({ id: project_id, name: 'Just Another Name' }, authorizer: authorizer)
      assert_predicate result, :failure?
      assert_equal Paleolog::Operation::UNAUTHENTICATED, result.error[:general]
    ...
    describe 'for authorized user' do
      ...
      it 'renames project' do
        result = operation.rename({ id: project_id, name: 'Other Name' }, authorizer: authorizer)
        assert_predicate result, :success?
        assert_equal 'Other Name', result.value.name
      end

      it 'can set the same name' do
        result = operation.rename({ id: project_id, name: 'Some Name' }, authorizer: authorizer)
        assert_predicate result, :success?
        assert_equal 'Some Name', result.value.name
      end

      it 'complains when name is nil' do
        result = operation.rename({ id: project_id, name: nil }, authorizer: authorizer)
        assert_predicate result, :failure?
        assert_equal :blank, result.error[:name]
      end

      it 'complains when name is blank' do
        result = operation.rename({ id: project_id, name: '   ' }, authorizer: authorizer)
        assert_predicate result, :failure?
        assert_equal :blank, result.error[:name]
      end

      it 'complains when name already exists' do
        result = operation.create({ name: 'Other Name', user_id: user.id }, authorizer: HappyAuthorizer.new)
        assert_predicate result, :success?
      ...
      it 'complains when name is too long' do
        max = 255
        result = operation.rename({ id: project_id, name: 'a' * (max + 1) }, authorizer: authorizer)
        assert_predicate result, :failure?
        assert_equal :too_long, result.error[:name]
      end
      ...
      it 'accepts name of max length' do
        name = 'a' * 255
        result = operation.rename({ id: project_id, name: name }, authorizer: authorizer)
        assert_predicate result, :success?
        assert_equal name, result.value.name
      end
